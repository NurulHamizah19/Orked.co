...
$select = $pdo->prepare("SELECT sum(total) AS t, count(invoice_id) AS inv FROM tbl_invoice WHERE agentid=$agentid");
$select->execute();
...
$net_total = $row->t;
    ...
    $select=$pdo->prepare("SELECT order_date, total FROM tbl_invoice WHERE agentid=$agentid GROUP BY order_date LIMIT 20");
    $select->execute();
  ...
  <div class="content-wrapper">
    ...
    <div class="box-body">
        ...
        <?php
            ...
            $select = $pdo->prepare("SELECT sum(comms) as comms FROM tbl_invoice WHERE agentid = $agentid");
            $select->execute();
            $row = $select->fetch(PDO::FETCH_OBJ);

            $total_comms = $row->comms;
       ...
       <div class="row">
            ...
            <table id="orderlisttable" class="table table-striped">
                    ...
                    <tbody>
                        <?php
                        $select = $pdo->prepare("SELECT * FROM tbl_invoice WHERE agentid=$agentid ORDER BY invoice_id DESC LIMIT 10");
                        $select->execute();
                        while($row = $select->fetch(PDO::FETCH_OBJ)){
                            $cid = $row->customer_name;
                            echo '
                            <tr>
                            <td><a href="editorder.php?id='.$row->invoice_id.'">'.$row->invoice_id.'</a></td>'.client_data($pdo,$cid);
                            echo'
