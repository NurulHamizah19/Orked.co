            ...
            <div class="box-body">
               <table class="table table-hover">
               	...
               	<tbody>
               		...
               		<?php 
               		$select = $pdo->prepare("SELECT *, DATE_FORMAT(tdate, '%d/%m/%Y') AS tdate FROM bank_ac WHERE dc='Debit' AND (tdate BETWEEN :fromdate AND :todate)");
               ...
               <hr>
               <?php 
				$select = $pdo->prepare("SELECT sum(d_amount) AS debit, sum(c_amount) AS credit FROM bank_ac WHERE tdate BETWEEN :fromdate AND :todate");
				$select->bindParam(':fromdate',$_POST['date_1']);
        		$select->bindParam(':todate',$_POST['date_2']);
				$select->execute();
        ...
        <div class="col-md-6">
            ...
            <div class="box-body">
               <table class="table table-hover">
               	...
               	<tbody>
               		<?php 
               		$select = $pdo->prepare("SELECT *, DATE_FORMAT(tdate, '%d/%m/%Y') AS tdate FROM bank_ac WHERE dc='Credit' AND (tdate BETWEEN :fromdate AND :todate)");
					$select->bindParam(':fromdate',$_POST['date_1']);
        			$select->bindParam(':todate',$_POST['date_2']);
					$select->execute();
					while($row = $select->fetch(PDO::FETCH_OBJ)){
               ...
               <hr>
               <?php 
				$select = $pdo->prepare("SELECT sum(c_amount) AS credit FROM bank_ac WHERE tdate BETWEEN :fromdate AND :todate");
				$select->bindParam(':fromdate',$_POST['date_1']);
        		$select->bindParam(':todate',$_POST['date_2']);
				$select->execute();
				$row = $select->fetch(PDO::FETCH_OBJ);
