...
if(isset($_POST['btnsaveorder'])){
    ...
    $payment_type = $_POST['txtpmethodval'];
    $delivery = "0";
    $profit = $_POST['txtprofit'];
    $print = $_POST['txtprint'];
    $createdo = "0";
    $pts = $_POST['txtpoint'];
    $getpts = round($total, 0);
    $remark = "-";
    ...
    $update_pt = $pdo->prepare("UPDATE tbl_client SET point = point - '$pts' WHERE id = '$customer_name'");
    $update_pt->execute();
    $add_pt = $pdo->prepare("UPDATE tbl_client SET point = point + '$getpts' WHERE id = '$customer_name'");
    $add_pt->execute();
      ...
      <div class="row">
        <div class="col-md-8">
          <div class="box">
            ...
            <h3 class="box-title"><a href="orderlist.php" class="btn btn-primary" role="button">Back</a></h3>
            </div>
            <!-- /.box-header -->
          ...
          <div class="box">
            ...
            <div class="box-body">
                ...
                <div align="center">
                    ...
                    <input type="button" onclick="payCash()" value="Cash (Ctrl)" class="btn btn-info">
                    <input type="button" onclick="payCard()" value="Card (Caps Lock)" class="btn btn-primary">
                    <input type="button" onclick="payEW()" value="E-Wallet (Shift)" class="btn btn-success">
        ...
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Pay with <span id="txtpmethod"></span></h4>
            ...
            <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="txtpoint" id="txtpoint" autofocus required>
            ...
            <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="txtpaid" id="txtpaid" autofocus required>
                  <input type="hidden" class="form-control" name="txtpmethodval" id="txtpmethodval" required>
                    </div>
            </div>
            ...
            <label>Print Receipt</label>
              <div class="form-group">
                <div class="input-group">
                  ...
                  <select class="form-control" name="txtprint" id="txtprint" required>
            ...
            <div class="modal-footer justify-center">
              ...
              <button type="submit" name="btnsaveorder" class="btn btn-primary">Save order</button>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
...
<script>
    function payCash(){
        var pmethod = $('#txtpmethod').text("Cash");
        document.getElementById('txtpmethodval').value = "Cash"
        $('#modal-cash').modal('show')
        $('#modal-cash').on('shown.bs.modal', function() {
          $('#txtpaid').trigger('focus');
        });
    }
    function payCard(){
        var pmethod = $('#txtpmethod').text("Card");
        // var pmethod_ip = $('#txtpmethodval').val("Card");
        document.getElementById('txtpmethodval').value = "Card"
        $('#modal-cash').modal('show')
        $('#modal-cash').on('shown.bs.modal', function() {
          $('#txtpaid').trigger('focus');
        });
    ...
    function payEW(){
        var pmethod = $('#txtpmethod').text("E-Wallet");
        // var pmethod_ip = $('#txtpmethodval').val("E-Wallet");
        document.getElementById('txtpmethodval').value = "E-Wallet"
        $('#modal-cash').modal('show')
        $('#modal-cash').on('shown.bs.modal', function() {
          $('#txtpaid').trigger('focus');
        });
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btnadd', function() {
        ...
        html += '<input type="hidden" class="form-control stock" name="stock[]" readonly>';
    ...
    function calculate(dis, paid, pts) {
        var subtotal = 0;
        var tax = 0;
        var discount = dis;
        var net_total = 0;
        ...
        $("#txtpaid").keyup(function() {
            ...
            calculate(discount, paid, point);
        })

        tax = (<?php echo $data->tax; ?> * 0.01) * subtotal;
        net_total = tax + subtotal;
        net_total = (net_total - discount - (point/100));
